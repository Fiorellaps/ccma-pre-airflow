...
#from functions.task_group_execute_hive_file import execute_hive_file
...
with DAG(
   ...
   tags=[global_dag_config["job_name"], global_dag_config["owner"], "s3", "jar"]
    ...
    hive_config_kantar_repair_tables = {
    ...
    hive_execute_kantar_repair_tables = execute_hive_hql(
                                        dag=dag, 
                                        config=hive_config_kantar_repair_tables, 
                                        )
    ...
    hive_config_incremental_graella_kantar = {
        "query_file_path": "enterprise/zapping/queries/insert_incremental_graella_kantar.hql",
        "query_bucket_name": ENTORNO # ccma-pre | ccma-pro
    }
    hive_config_incremental_graella_kantar["query_name"] = hive_config_incremental_graella_kantar['query_file_path'].split('/')[-1].split('.hql')[0].replace('_', '').replace(' ', '').lower()
    
    hive_execute_incremental_graella_kantar = execute_hive_hql(
                                        dag=dag, 
                                        config=hive_config_incremental_graella_kantar, 
                                        )
    
    #  Insert incremental kantar sortides
    hive_config_incremental_kantar_sortides = {
        "query_file_path": "enterprise/zapping/queries/insert_incremental_kantar_sortides.hql",
        "query_bucket_name": ENTORNO # ccma-pre | ccma-pro
    }
    hive_config_incremental_kantar_sortides["query_name"] = hive_config_incremental_kantar_sortides['query_file_path'].split('/')[-1].split('.hql')[0].replace('_', '').replace(' ', '').lower()
    
    hive_execute_incremental_kantar_sortides = execute_hive_hql(
                                        dag=dag, 
                                        config=hive_config_incremental_kantar_sortides, 
                                        )

    #  Insert incremental kantar rebots
    hive_config_incremental_kantar_rebots = {
        "query_file_path": "enterprise/zapping/queries/insert_incremental_kantar_rebots.hql",
        "query_bucket_name": ENTORNO # ccma-pre | ccma-pro
    }
    hive_config_incremental_kantar_rebots["query_name"] = hive_config_incremental_kantar_rebots['query_file_path'].split('/')[-1].split('.hql')[0].replace('_', '').replace(' ', '').lower()
    
    hive_execute_incremental_kantar_rebots = execute_hive_hql(
                                        dag=dag, 
                                        config=hive_config_incremental_kantar_rebots, 
                                        )
    
    #  Insert incremental kantar abandonament
    hive_config_incremental_kantar_abandonament = {
        "query_file_path": "enterprise/zapping/queries/insert_incremental_kantar_abandonamet.hql",
        "query_bucket_name": ENTORNO # ccma-pre | ccma-pro
    }
    hive_config_incremental_kantar_abandonament["query_name"] = hive_config_incremental_kantar_abandonament['query_file_path'].split('/')[-1].split('.hql')[0].replace('_', '').replace(' ', '').lower()
    
    hive_execute_incremental_kantar_abandonament = execute_hive_hql(
                                        dag=dag, 
                                        config=hive_config_incremental_kantar_abandonament, 
                                        )
    ...
    hive_execute_kantar_repair_tables >> 
    hive_execute_incremental_graella_kantar >> 
    hive_execute_incremental_kantar_sortides >> 
    hive_execute_incremental_kantar_rebots >> 
    hive_execute_incremental_kantar_abandonament >>
    success_email)