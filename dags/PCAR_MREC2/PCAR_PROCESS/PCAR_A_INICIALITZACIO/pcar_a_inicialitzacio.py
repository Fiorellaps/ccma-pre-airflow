...
def pcar_a_inicialitzacio(dag: DAG, config, current_path="") -> TaskGroup:
    ...
    with TaskGroup(
        ...
        # carrega_settings_base
        carrega_settings_base = PythonOperator(
            task_id='carrega_settings_base',
        ...
        hive_execute_consolidacio_A3_setting_dades = execute_hive_hql(
                                            dag=dag, 
                                            config=hive_config_consolidacio_A3_setting_dades, 
                                            )
        ...
        hive_execute_consolidacio_A4_raw_bloc_aux = execute_hive_hql(
                                            dag=dag, 
                                            config=hive_config_consolidacio_A4_raw_bloc_aux, 
                                            )
        

        # carrega_settings_dades
        carrega_settings_dades = PythonOperator(
            task_id='carrega_settings_dades',
            python_callable=to_do,
            op_kwargs={
                "param": "",
            },
